  ...
  return Widget.Box({
    ...
    children: [
      (icon != 'NO_ICON' 
        ...
        }) : MaterialIcon(`${notifObject.urgency == 'critical' 
            ? 'release_alert' : guessMessageType(notifObject.summary.toLowerCase())}`, 
            'hugerass', { hexpand: true })
      )
  ]})
}
    ...
    const notificationBox = Widget.Box({
        ...
        connections: [
          [gesture, self => {
            ...
            if (offset > 0) {
              if (initialDir < 0) self.setCss(`margin-left: 0px; margin-right: 0px;`)
              else self.setCss(`
