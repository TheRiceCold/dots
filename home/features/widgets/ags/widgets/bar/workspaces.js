    ...
    Widget.Box({
      ...
      connections: [[
        Hyprland.active.workspace, box => {
          ...
          box.setCss(`margin-left: ${1.774 * (ws - 1) + 0.068}rem;`);
          lastWorkspace = ws;
        }
      ]],
      children: [
        Widget.Label({ vpack: 'center', className: 'bar-ws-active', label: `•` })
      ]
    })
  ]
});
    ...
    children: [
      Widget.Overlay({
        ...
        child: Widget.Box({
          ...
          children: [Widget.Box({ className: 'bar-group-standalone bar-group-pad' })]
        }),
        overlays: [
          Widget.Overlay({
            setup: (self) => {
            ...
            child: Widget.Box({
                ...
                children: Array.from({ length: NUM_OF_WORKSPACES }, (_, i) => i + 1).map(i => Widget.Button({
                  ...
                  child: Widget.Label({ vpack: 'center', label: `${i}`, className: 'bar-ws txt' }),
                })),
