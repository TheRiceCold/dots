...
const { Box, Button, Entry, Icon, Label, Revealer, Scrollable } = Widget;
...
const chatGPTSettings = Revealer({
  ...
  revealChild: true,
  connections: [
    [ChatGPT, (self) => { self.revealChild = false; }, 'newMsg'],
    [ChatGPT, (self) => { self.revealChild = true; }, 'clear'],
  ],
  child: Box({
    vertical: true,
    hpack: 'fill',
...
const chatContent = Box({
  ...
  connections: [
    ...
    [ChatGPT, (box) => { box.children = [chatWelcome]; }, 'clear'],
    [ChatGPT, (box) => { box.children = [chatWelcome]; }, 'initialized'],
  ]
});
